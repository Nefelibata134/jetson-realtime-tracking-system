...
通过。该审计时间点早于任何 Jetson 系统推理；随后执行的正式轮次及未通过的事件精度门禁
见[`CAVIAR 外部事件验证结果`](caviar_external_validation_results.md)。
...
事件片段和标注视频。输入队列容量固定为 `8`，用于吸收 TensorRT 首帧冷启动抖动；持续
处理能力仍由实时回放、丢帧计数和目标帧数共同约束。常驻 `edge-vision.service` 活跃时
会拒绝运行，避免资源竞争。

### 首次执行的测量工具修订

`2026-09-01T15:30:18Z` 开始的首次 Jetson 留出执行使用输入队列容量 `2`。四段视频分别
记录到 `2/3/2/2` 个输入丢帧，最后一段只处理 `483/485` 帧；前三段虽然借助未标注尾帧
达到了目标数量，但处理帧序已不能与 XML 严格对齐。因此四个原始运行目录全部保留为
“测量工具无效轮次”，不汇总为正式模型指标。

`2026-09-01T15:36:42.093Z` 在不修改模型、TensorRT engine、检测/跟踪阈值、事件规则、
真值或匹配标准的前提下，将输入队列容量提高到 `8`。该修订只吸收约 `45 ms` 的首帧
冷启动抖动；正式轮次要求每段 `dropped_total=0`、`sequence_gaps=0` 且
`target_reached=true`，否则仍判定为测量无效，不计算事件得分。
